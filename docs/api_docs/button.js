/**
 * A button is essentially a [Sprite](/api/sprite) with a [Text](/api/text) child with added functionality to support keyboard interaction. To create a button, pass any Sprite properties and pass Text properties using the `text` parameter. You can also pass any [pointer](/api/pointer) event functions.
 *
 ...
 *
 * - `focused` - The button is focused by the keyboard, usually by pressing the <kbd>Tab</kbd> key.
 * - `hovered` - The button is hovered by the mouse.
 * - `pressed` - The button is pressed with a mouse or touch event or the <kbd>Enter</kbd> or <kbd>Spacebar</kbd> keys were pressed while the button has focus.
