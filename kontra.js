...
class Animation {
  ...
  render(properties) {
    ...
    let context = (properties.context || kontra$1.context);
    context.drawImage(
...
function GameLoop(properties) {
  ...
  let clear = (properties.clearCanvas === false ?
              kontra$1._noop :
              function clear() {
                kontra$1.context.clearRect(0,0,kontra$1.canvas.width,kontra$1.canvas.height);
              });
  let last, rAF, now, dt;
...
function getObjectProto(object) {
  return kontra$1[object]
    ? kontra$1[object].prototype || kontra$1[object]
    : null;
}
...
function pointerHandler(e, event) {
  ...
  let ratio = kontra$1.canvas.height / kontra$1.canvas.offsetHeight;
  let rect = kontra$1.canvas.getBoundingClientRect();
  ...
  if (callbacks$2[event]) {
    callbacks$2[event](e, object);
  }
}
...
kontra$1.on('init', () => {
  kontra$1.canvas.addEventListener('mousedown', pointerDownHandler);
  kontra$1.canvas.addEventListener('touchstart', pointerDownHandler);
  kontra$1.canvas.addEventListener('mouseup', pointerUpHandler);
  kontra$1.canvas.addEventListener('touchend', pointerUpHandler);
  kontra$1.canvas.addEventListener('blur', blurEventHandler$1);
  kontra$1.canvas.addEventListener('mousemove', mouseMoveHandler);
  kontra$1.canvas.addEventListener('touchmove', mouseMoveHandler);
});
...
class Quadtree {
  ...
  constructor(properties) {
    ...
    this.bounds = properties.bounds || {
      x: 0,
      y: 0,
      width: kontra$1.canvas.width,
      height: kontra$1.canvas.height
    };
...
class Sprite {
  ...
  init(properties, prop, temp, firstAnimation) {
    ...
    this.position = VectorFactory(properties.x, properties.y);
    this.velocity = VectorFactory(properties.dx, properties.dy);
    this.acceleration = VectorFactory(properties.ddx, properties.ddy);

    // defaults
    this.width = this.height = this.rotation = 0;
    this.ttl = Infinity;
    this.anchor = {x: 0, y: 0};
    this.context = kontra$1.context;
...
const TileEngine = function(properties) {
  ...
  let tileEngine = Object.assign({
    ...
    set sx(value) {
      this._sx = Math.min( Math.max(0, value), mapwidth - kontra$1.canvas.width );
    },

    set sy(value) {
      this._sy = Math.min( Math.max(0, value), mapheight - kontra$1.canvas.height );
    },
  ...
  tileEngine.tilesets.forEach(tileset => {
    let url = (kontra$1.assets ? kontra$1.assets._d.get(properties) : '') || window.location.href;
    ...
    if (tileset.source) {
      ...
      let source = kontra$1.assets.data[kontra$1.assets._u(tileset.source, url)];
    ...
    if (''+tileset.image === tileset.image) {
      ...
      let image = kontra$1.assets.images[kontra$1.assets._u(tileset.image, url)];
  ...
  function render(canvas) {
    (tileEngine.context || kontra$1.context).drawImage(
      canvas,
      tileEngine.sx, tileEngine.sy, kontra$1.canvas.width, kontra$1.canvas.height,
      0, 0, kontra$1.canvas.width, kontra$1.canvas.height
    );
  }
...
kontra$1.assets = assets$1;
kontra$1.keys = keys;
kontra$1.plugin = plugin;
kontra$1.pointer = pointer$1;
kontra$1.store = store;

kontra$1.animation = AnimationFactory;
kontra$1.gameLoop = GameLoop;
kontra$1.pool = PoolFactory;
kontra$1.quadtree = QuadtreeFactory;
kontra$1.sprite = SpriteFactory;
kontra$1.spriteSheet = SpriteSheetFactory;
kontra$1.tileEngine = TileEngine;
kontra$1.vector = VectorFactory;
