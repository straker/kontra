  ...
  describe('render', () => {
    ...
    if (hasAnchor) {
      it('should take into account gameObject.anchor', () => {
        ...
        sinon.stub(gameObject, '_d').callsFake(noop);

        gameObject.render();

        expect(gameObject._d.calledWith(-50, -25)).to.be.ok;

        gameObject.anchor = {x: 1, y: 1};
        gameObject.render();

        expect(gameObject._d.calledWith(-100, -50)).to.be.ok;

        gameObject._d.restore();
      });
    }
